...
@section('title', 'Borang Pesanan Pengeluaran Stok (KEW.PS-8)')
@section('form_code', 'KEW.PS-8')

@section('content')
<div class="space-y-4">
    <div class="text-center pb-2 border-b border-slate-300">
        <h2 class="text-sm font-bold uppercase tracking-wider text-slate-900">BORANG PESANAN PENGELUARAN STOK</h2>
        <p class="text-[11px] text-slate-600">(Tatacara Pengurusan Stor AM 6.5 - No. Pesanan: <span class="font-mono font-bold">{{ $stockRequest->request_number ?? 'PS8/ASM/2026/0001' }}</span>)</p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 text-xs">
        <div class="space-y-1">
            ...
            <div><span class="font-bold">Stor Membekal:</span> {{ $stockRequest->store->name ?? 'Stor Utama' }}</div>
            <div><span class="font-bold">Bahagian / Unit Pemohon:</span> {{ $stockRequest->requester->department ?? '-' }}</div>
            <div><span class="font-bold">Tujuan Permohonan:</span> {{ $stockRequest->purpose ?? '-' }}</div>
        </div>
        <div class="space-y-1 text-right">
            ...
            <div><span class="font-bold">Nama Pemohon:</span> {{ $stockRequest->requester->name ?? '-' }} ({{ $stockRequest->requester->staff_id ?? '' }})</div>
    ...
    <div class="grid grid-cols-4 gap-4 pt-6 text-[11px]">
        <div class="border border-slate-300 p-2.5 rounded">
            ...
            <div class="h-12"></div>
            <div>Nama: {{ $stockRequest->requester->name ?? 'Pemohon' }}</div>
            <div>Jawatan: {{ $stockRequest->requester->position ?? '-' }}</div>
        ...
        <div class="border border-slate-300 p-2.5 rounded">
            ...
            <div>Jawatan: {{ $stockRequest->approver->position ?? '-' }}</div>
            <div>Tarikh: {{ $stockRequest->approved_at ? \Carbon\Carbon::parse($stockRequest->approved_at)->format('d/m/Y') : '-' }}</div>
        </div>

        <div class="border border-slate-300 p-2.5 rounded">
            <div class="font-bold border-b border-slate-200 pb-1 mb-2">3. Pegawai Pengeluar</div>
            <div class="h-12"></div>
            <div>Nama: {{ $stockRequest->issuer->name ?? 'Pegawai Stor' }}</div>
            <div>Jawatan: {{ $stockRequest->issuer->position ?? 'Pegawai Stor' }}</div>
        ...
        <div class="border border-slate-300 p-2.5 rounded">
            ...
            <div>Jawatan: {{ $stockRequest->recipient->position ?? '-' }}</div>
